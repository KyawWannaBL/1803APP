...
export function ScreenPage() {
  ...
  useEffect(() => {
    async function reload() {
      ...
      setBusy(true);
      try {
        const [portalSummary, screenDatasets] = await Promise.all([fetchPortalSummary(user?.role ?? 'EA'), fetchScreenDatasets(screen, 12)]);
      ...
      <div className="page-grid">
        <div className="page-main-stack">
          {screen.tables.slice(0,2).map((table) => (
            <DatasetPanel key={table} table={table} rows={datasets[table] ?? []} busy={busy} onReload={() => undefined} selectedId={selectedRecord ? String(selectedRecord.id) : null} onSelect={setSelectedRecord} />
          ))}
          {entityType ? <WorkflowGatePanel entityType={entityType as WorkflowEntityType} record={selectedRecord} onTransitioned={() => undefined} /> : null}
        </div>
