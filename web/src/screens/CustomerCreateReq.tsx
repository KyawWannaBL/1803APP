...
import { Plus, Activity, Zap, ShieldCheck, Database, RefreshCw, UploadCloud, Map, FileSpreadsheet, ScanLine, PenTool, Printer, Truck, FileText, Package, CheckCircle2, AlertTriangle, Wallet, Building2, ArrowDownRight, Inbox, AlertCircle, ArrowUpRight, Monitor, PlusCircle, Users, UserCheck, XCircle, Flame, DollarSign, Banknote, ListChecks, Coins, BookOpen, RefreshCcw, Headphones, Search, History, Edit3, AlertOctagon, Book, ShoppingBag, PlusSquare, RotateCcw, Receipt, Settings, Code, Home, MessageSquare, User, Sliders, Terminal, Server, BarChart3 } from 'lucide-react';
...
export default function CustomerCreateReq() {
  ...
  return (
    <div className="p-8 space-y-8 animate-in fade-in duration-500 max-w-[1600px] mx-auto text-white">
      <div className="relative group acrylic-sheet acrylic-blue rounded-[3rem] p-10 flex flex-col md:flex-row items-center gap-8 shadow-2xl overflow-hidden border border-white/10">
        <div className="absolute top-0 right-0 w-96 h-96 bg-blue-500/10 rounded-full blur-[100px] -mr-32 -mt-32" />
        <div className="p-6 bg-blue-500/20 rounded-2xl border border-blue-500/30 relative z-10">
           <Plus className="h-10 w-10 text-blue-400" />
        </div>
        <div className="flex-1 relative z-10">
          <h2 className="text-4xl font-black uppercase italic tracking-tighter leading-none text-white">Create Request</h2>
          <div className="text-blue-400 text-[10px] mt-4 flex items-center gap-4 font-bold uppercase tracking-widest">
      ...
      <div className="acrylic-sheet rounded-[3rem] bg-slate-900/90 border-white/20 p-10 min-h-[500px] flex flex-col items-center justify-center">
         {loading ? (
            <div className="text-center space-y-4">
               <Activity size={48} className="text-blue-500/50 animate-bounce mx-auto" />
            ...
            <div className="text-center w-full max-w-2xl space-y-8">
               ...
               {'GRID' === 'BULK' && (
                 <div className="space-y-6">
                   <UploadCloud size={64} className="mx-auto text-blue-400/50" />
                   <p className="font-bold text-slate-300">Drag and drop Excel (.xlsx) file here</p>
                   <input type="file" className="block w-full text-slate-400 file:py-3 file:px-6 file:rounded-xl file:bg-blue-600 file:text-white cursor-pointer bg-white/5 rounded-xl border border-white/10" />
                 </div>
               )}

               {('GRID' === 'SCAN' || 'GRID' === 'OCR') && (
                 <div className="space-y-6">
                   <ScanLine size={64} className="mx-auto text-blue-400/50" />
                   <div className="aspect-video bg-black rounded-2xl border border-white/20 relative overflow-hidden flex items-center justify-center">
                     <div className="h-1 w-full bg-blue-500/50 absolute top-0 animate-[scan-line_3s_linear_infinite]" />
               ...
               {('GRID' === 'POD' || 'GRID' === 'SIGN') && (
                 <div className="w-full space-y-4">
                   ...
                   <div className="flex gap-4">
                     ...
                     <button onClick={handleAction} className="flex-1 py-3 bg-blue-600 rounded-xl font-black uppercase text-white">SUBMIT E-POD</button>
                   </div>
                 </div>
               )}
               ...
               {'GRID' === 'PRINT' && (
                 <div className="w-full space-y-6">
                   <div className="flex gap-4 justify-center">
                      ...
                      <button onClick={handleAction} className="px-8 py-3 bg-blue-600 rounded-xl font-black text-white">PRINT BATCH</button>
                   </div>
               ...
               {('GRID' === 'GRID' || 'GRID' === 'DASH') && (
                 <table className="w-full text-xs text-left">
                    <thead className="text-blue-400/70 border-b border-white/10"><tr><th className="pb-2">ID</th><th className="pb-2">Status</th><th className="pb-2">Update</th></tr></thead>
               ...
               {('GRID' === 'MAP') && (
                 <div className="aspect-video bg-slate-900/90 rounded-3xl flex items-center justify-center border border-white/10">
                   <Map size={64} className="text-blue-400/50 animate-pulse" />
                 </div>
               )}
            </div>
         )}
